...
static uint32_t *stack_new( task_type_t task_type );
...
static uint32_t *stack_new(task_type_t task_type) {
    static uint32_t next_stack_k = 0xa0f00000;		//a0f00000
	static uint32_t next_stack_u = 0x300000;
    
	
	if(task_type == KERNEL_THREAD){
		next_stack_k += 0x10000;
		return (uint32_t *) next_stack_k;
	}
	else{
		next_stack_u += 0x10000;
		return (uint32_t *) next_stack_u;
	}
...
static void initialize_pcb( pcb_t *p, pid_t pid, struct task_info *ti ) {
    ...
    case KERNEL_THREAD:
        thn++;
        p->kernel_tf.regs[ 29 ] = (uint32_t) stack_new(KERNEL_THREAD);
        p->nested_count         = 1;
        p->page_table           = 0;
        break;
    case PROCESS:
        procn++;
        p->kernel_tf.regs[ 29 ] = (uint32_t) stack_new(KERNEL_THREAD);
        p->user_tf.regs[ 29 ]   = (uint32_t) stack_new(KERNEL_THREAD);
        p->nested_count         = 0;
        // TODO: p5 here!
        p->size                 = ti->size;
        p->loc                  = ti->loc;
		//print_hex(10, 24, p->user_tf.regs[ 29 ]);
        p->page_table           = setup_page_table(pid);
        break;
    default:
        ASSERT( FALSE );
    }
	//if(pid == 1)	p->entry_point = 0xa0909000;
	//else	if(pid == 2)	p->entry_point = 0xa091c000;
