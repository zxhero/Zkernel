...
void loader(char *file,FILE *fp2) {
	...
	for(i = 0, ph = (void *)buf2 + elf->e_phoff; i < elf->e_phnum; i ++) {
		...
		if(ph[i].p_type == PT_LOAD) {
			...
			addr = ph[i].p_offset;
			if(ph[i].p_vaddr == 0x100000){
				addr_to_mem = 0xa0806600 - 0xa0800000;
			}
			else if(ph[i].p_vaddr == 0x200000){
				addr_to_mem = 0xa0818600 - 0xa0800000;
			}
			else	addr_to_mem = ph[i].p_vaddr - 0xa0800000;
			fseek(fp,addr,SEEK_SET);
			fseek(fp2,addr_to_mem,SEEK_SET);
			while(filesz >= 512){
				for(j = 0;j < 512; j++){
					buf[j] = 0;
				}
			...
			total_sector = sector_mem + (addr_to_mem>>9);//0x130
			printf("Sectors: %u\n",sector_mem);
			printf("File will be loaded to %x\n",ph[i].p_vaddr);
			printf("The file size of %s is %x\n",file,ph[i].p_filesz);
			printf("The memory size of %s is %x\n",file,ph[i].p_memsz); 
		}
	}
	fclose(fp);
}
