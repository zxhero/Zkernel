...
void do_exit(){
	pcb_t *pcb = NULL; 
	 ...
	 pcb = (pcb_t *)dequeue(&current_running->wait_queue);
	while(pcb != NULL){
		unblock(pcb);
		pcb = (pcb_t *)dequeue(&current_running->wait_queue);		
	}
	// while(unblock_one(&current_running->wait_queue));
     scheduler_entry();
...
void block(node_t * wait_queue){
	//print_hex(10, 24, current_running->entry_point);
	 ...
	 current_running->wait_in = wait_queue;
	//print_hex(10, 24, current_running->wait_in);
	//print_hex(10, 24, &(current_running->wait_queue));
     scheduler_entry();
     enter_critical();
}
