...
void mutex_init(mutex_t l){
	invoke_syscall(SYSCALL_LOCK_INIT, (int) l , IGNORE, IGNORE);
}

int	mutex_acquire(mutex_t  l){
	return invoke_syscall(SYSCALL_LOCK_ACQ, (int) l , IGNORE, IGNORE);
}

void mutex_release(mutex_t l){
	invoke_syscall( 	SYSCALL_LOCK_REL, (int) l , IGNORE, IGNORE);
}
