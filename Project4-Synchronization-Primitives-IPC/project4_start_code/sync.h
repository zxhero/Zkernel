...
/* TODO */
typedef struct condition{//queue + lock_t+queue
	node_t	pro_wait_queue;
} condition_t;

/* TODO */
typedef struct semaphore{//unsignedint value + queue
	uint32_t	value;
	node_t	pro_wait_queue;
	lock_t	mutex;
} semaphore_t;
...
/* TODO */
typedef struct barrier{//unsignedint value  + queue
	uint32_t	total_num;
	uint32_t	cur_num;
	node_t		pro_wait_queue;
	lock_t		mutex;
